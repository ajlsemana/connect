...
class BlueConnectController extends BaseController {
	private $data = array();

	protected $layout = "layouts.blueconnect";
	...
	public function registerProcess() {		
		...
		if ($validator->fails()) {
			...
			$subject = 'Blue Mena Group: Welcome, '.$name;
			$link = '<br><br><a target="_blank" href="'.URL::to('login').'">Click here</a> to activate your account and log in to our Blue Mena Portal.';
			...
			$headers = 'Content-type:text/html;charset=UTF-8';							
			$mail_submit = mail($to, $subject, 'Username: <strong>'.Input::get('username').'</strong><br>Password: <strong>'.$password.'</strong>'.$link, $headers);			
			if(! $mail_submit) {
				return Redirect::to('registration')
				->with('error', 'Failed to send email message.')
				->withInput();		
				...
				return Redirect::to('registration')
				->with('success', 'Your account is created successfully.<br>Kindly check your primary email for your <b>password</b> and <b>account verification</b>.');
			}		
		}
	}
