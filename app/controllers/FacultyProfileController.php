...
class FacultyProfileController extends BaseController {
	...
	public function updateProfilePasswordData() {	
		...
		if(empty($old) AND empty($new) AND empty($con)) {			
			...
			return Redirect::to('trainer/change_password')
					->with('error', $message)
					->withInput();	
		} else {
	...
	protected function getProfilePasswordForm() {
		...
		$this->breadcrumbs[] = array(
									'href'      => URL::to('trainer/change_password'),
		...
		$this->data['url_cancel'] = URL::to('trainer/dashboard');
		
		$this->layout->content = View::make('faculty.password', $this->data);
	}
	...
	protected function getProfileForm() {
		...
		$this->layout->content = View::make('faculty.profile', $this->data);			
	}
}