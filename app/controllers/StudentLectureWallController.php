...
class StudentLectureWallController extends BaseController {
	private $data = array();
	...
	protected function getForm() {	
		...
		$this->data['module'] = 'Announcements';

		$this->data['menu'] = View::make('student.menu_tabs', $this->data);		
		$this->layout->content = View::make('student.class_wall', $this->data);
	}
	...
	public function postIt() {		
		...
		if (! empty($message)) {			
			return Redirect::to('trainee/wall-announcement/'.$sid)
					->with('error', $message)
					->withInput();	
		} else {
			$uid = Auth::user()->id;
			...
			foreach($emails as $email) {
				...
				$body .= $post_msg;
				#$body .= '<br><br><a href="'.URL::to('trainee/wall-announcement/' . $sid).'">View Announcement</a>';
			...
			return Redirect::to('trainee/wall-announcement/' . $sid.$status)
