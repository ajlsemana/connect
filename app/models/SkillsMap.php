<?php
class SkillsMap extends Eloquent {
	/**
	 * The database table used by the model.
	 *
	 * @var string
	 */
	protected $table = 'skills_map';

	public static function getUsers($data = array()) {
		$query = DB::table('users')
			->select(DB::raw('certification_color_v8.color AS v8_color, certification_color_v7.color AS v7_color, skills_map_v7.*, skills_map_v7.status AS status_rate_v7, skills_map.*, skills_map.status AS status_rate,users.*'))		
			->leftjoin('skills_map', 'users.id', '=', 'skills_map.uid')
			->leftjoin('skills_map_v7', 'users.id', '=', 'skills_map_v7.uid')
			->leftjoin('certification_color_v7', 'users.id', '=', 'certification_color_v7.uid')
			->leftjoin('certification_color_v8', 'users.id', '=', 'certification_color_v8.uid')
		...
		if( CommonHelper::hasValue($data['filter_company']) ) $query->where('users.company', '=', $data['filter_company']);
		...
		$color_picker = array();
		$color_picker['red'] = array('red', 'green', 'yellow', 'blue');
		$color_picker['green'] = array('green', 'yellow', 'blue');
		$color_picker['yellow'] = array('yellow', 'blue');
		$color_picker['blue'] = array('blue');

		if(isset($data['filter_v7']) && isset($data['filter_v8'])) {			
			if($data['filter_v7'] != '' && $data['filter_v8'] != '') {
				$query->whereIn('certification_color_v7.color', $color_picker[$data['filter_v7']]);			
				$query->whereIn('certification_color_v8.color', $color_picker[$data['filter_v8']]);			
			}
		} else {
			if(isset($data['filter_v7'])) {
				$query->whereIn('certification_color_v7.color', $color_picker[$data['filter_v7']]);
			}
			
			if(isset($data['filter_v8'])) {
				$query->whereIn('certification_color_v8.color', $color_picker[$data['filter_v8']]);			
			}
		}
	...
	public static function getTotalEngineers($data = array()) {
		...
		if( CommonHelper::hasValue($data['filter_primary_email']) ) $query->where('primary_email_address', 'LIKE', '%'.$data['filter_primary_email'].'%');
		if( CommonHelper::hasValue($data['filter_user_type']) ) $query->where('user_type', '=', $data['filter_user_type']);
		if( CommonHelper::hasValue($data['filter_company']) ) $query->where('company', '=', $data['filter_company']);
	...
	public static function getSkillsMap($uid = 0) {
		$query = DB::table('skills_map')
			->select(DB::raw('skills_map.*, users.position, users.company, users.profile_pic, CONCAT(users.first_name, " ", users.last_name) AS fullname'))		
			->leftjoin('users', 'users.id', '=', 'skills_map.uid')
			->where('users.id', '=', $uid);			
	...
	public static function colorStatus($uid = 0, $current_color = 'red') {
		...
		if(
			...
			&& ($data->altitude_voice >= 3 || $data->altitude_email >= 3 || $data->altitude_chat >= 3 || $data->social >= 3)
			&& ($data->altitude_desktop >= 3 || $data->altitude_ivr >= 3 || $data->altitude_express_routing >= 3 || $data->altitude_integration >= 3 || $data->altitude_workflow >= 3)
			&& ($data->uci_installation >= 3) 
			&& ($data->uci_patch >= 3)
			&& ($data->reporting_framework >= 3)
		) {		
				...
				$layer4_explode = explode('|', $data->altitude_voice.'|'.$data->altitude_email.'|'.$data->altitude_chat.'|'.$data->social);
				$layer5_explode = explode('|', $data->altitude_desktop.'|'.$data->altitude_ivr.'|'.$data->altitude_express_routing.'|'.$data->altitude_integration.'|'.$data->altitude_workflow); 				
				...
				$layer8 = $data->reporting_framework;

				$average = ($layer1_explode[0] + $layer2_explode[0] + $layer3_explode[0] + $layer4_explode[0] + $layer5_explode[0] + $layer6 + $layer7 + $layer8) / 8;
		...
		if($current_color != $new_color) {
			$query_color = DB::table('certification_color_v8')->where('uid', '=', $uid);
	...
	public static function getSkillsMapGraph($uid = 0) {
		$query = DB::table('skills_map')
			->select(DB::raw('skills_map.*, users.position, users.company, users.profile_pic, CONCAT(users.first_name, " ", users.last_name) AS fullname'))		
			->leftjoin('users', 'users.id', '=', 'skills_map.uid')
			->where('users.id', '=', $uid);			
	...
	public static function updateSkillsMap($id, $data = array(), $uid) {	
		...
		$query_history = DB::table('skills_map_update')->where('uid', '=', $uid);		
		$result_history = $query_history->first();

		$query_orig= DB::table('skills_map')->where('id', '=', $id);		
		...
		*/
		...
		if($result_history->reporting_framework != $data['reporting_framework']) {
			$param['reporting_framework'] = $data['reporting_framework'];
			$param['reporting_framework_date'] = date('Y-m-d H:i:s'); 
			$param['reporting_framework_by'] = $by;
		}
		...
		if($result_history->mobile_dashboard != $data['mobile_dashboard']) {
			$param['mobile_dashboard'] = $data['mobile_dashboard'];
			$param['mobile_dashboard_date'] = date('Y-m-d H:i:s'); 
			$param['mobile_dashboard_by'] = $by;
		}

		if($result_history->strategy_manager != $data['strategy_manager']) {
			$param['strategy_manager'] = $data['strategy_manager'];
			$param['strategy_manager_date'] = date('Y-m-d H:i:s'); 
			$param['strategy_manager_by'] = $by;
		}

		if($result_history->enterprise_recorder != $data['enterprise_recorder']) {
			$param['enterprise_recorder'] = $data['enterprise_recorder'];
			$param['enterprise_recorder_date'] = date('Y-m-d H:i:s'); 
			$param['enterprise_recorder_by'] = $by;
		}

		if($result_history->otcs != $data['otcs']) {
			$param['otcs'] = $data['otcs'];
			$param['otcs_date'] = date('Y-m-d H:i:s'); 
			$param['otcs_by'] = $by;
		}
		...
		$query_old = DB::table('skills_map_update')->where('uid', '=', $uid);
		$query_old->update($param);	

		$query = DB::table('skills_map')->where('id', '=', $id);
		$query->update($data);
	}
	...
	public static function getEnrolledCourses() {		
		$query = DB::table('registrants')
			->select(DB::raw('courses.*, registrants.uid, registrants.attendance_status'))
	...
	public static function addSkillProficiency($data = array()) {
		$id = DB::table('skills_proficiency_v8')->insertGetId($data);
		
		return $id;
	}

	public static function getSkillProficiencyData($uid = 0, $skill_name = '') {
		$query = DB::table('skills_proficiency_v8')		
			->select(DB::raw('
					skills_proficiency_v8.*, customers.logo	, customers.company, CONCAT(users.first_name, " ", users.last_name) AS updated_by				
				'))
			->leftjoin('customers', 'customers.id', '=', 'skills_proficiency_v8.cid')
			->leftjoin('users', 'users.id', '=', 'skills_proficiency_v8.admin_id')			
			->where('skills_proficiency_v8.skill_name', '=', $skill_name)
			->where('skills_proficiency_v8.uid', '=', $uid)
			->orderBy('skills_proficiency_v8.created_at', 'ASC');
		...
		$img_array = array(
			...
			'reporting_framework' => HTML::image('resources/img/skills-map/installation/reporting_framework.png', 'logo', array('width' => '89', 'height' => '24', 'class' => 'img-responsive')),

			'otcs' => HTML::image('resources/img/skills-map/connectors/otcs.png', 'logo', array('width' => '55', 'height' => '24', 'class' => 'img-responsive')),
			'strategy_manager' => HTML::image('resources/img/skills-map/connectors/strategy_manager.png', 'logo', array('width' => '65', 'height' => '24', 'class' => 'img-responsive')),
			'enterprise_recorder' => HTML::image('resources/img/skills-map/connectors/enterprise_recorder.png', 'logo', array('width' => '80', 'height' => '24', 'class' => 'img-responsive')),
			...
			'mobile_dashboard' => HTML::image('resources/img/skills-map/connectors/mobile_dashboard.png', 'logo', array('width' => '64', 'height' => '22', 'class' => 'img-responsive'))			
		);
		...
		if($ctr > 0) {
			...
			foreach($data as $row) {
				...
				if(Auth::user()->user_type == 1) {
					$html .= '<td><a style="cursor: pointer;" onclick="deleteSkillProficiency('.$row->id.');"><i class="icon-trash"></i></a></td>';				
				}
				$html .= '</tr>';
			}
			$html .= '</table>';
	...
	public static function getAllEngrCustomers($uid) {
		$query = DB::table('skills_proficiency_v8')		
			...
			->leftjoin('customers', 'customers.id', '=', 'skills_proficiency_v8.cid')
			->where('skills_proficiency_v8.uid', '=', $uid)
			->groupBy('skills_proficiency_v8.cid')
			->orderBy('skills_proficiency_v8.created_at', 'DESC');
	...
	public static function getVersionColor($uid) {
		$query = DB::table('certification_color_v8')			
			->where('uid', '=', $uid);			
	...
	public static function deleteSkill($id) {
		$query = DB::table('skills_proficiency_v8')->where('id', $id)->delete();
	}

	public static function getSkillDescription() {
		$query = DB::table('skill_descriptions');

		$data = $query->get();
		$arr = array();

		foreach($data as $row) {
			$arr[$row->skill_name] = $row->description;
		}

		return $arr;
	}	

	public static function getFeedbackLogo($uid = 0) {
		$query = DB::table('customers_feedback')
			->select(DB::raw('customers_feedback.*, customers.company, customers.logo'))		
			->leftjoin('customers', 'customers.id', '=', 'customers_feedback.cid')
			->where('customers_feedback.uid', '=', $uid)
			->groupBy('customers_feedback.cid')
			->orderBy('customers.company', 'ASC');			
