...
<a href="{{ URL::to('trainee/engineers') }}" class="btn"><i class="icon-large icon-arrow-left"></i></a>
@endif
...
{{ Form::model($skillsV7, array('url'=>'', 'class'=>'form-horizontal', 'id'=>'', 'role'=>'form', 'method' => 'post')) }}
<div class="rows">
   <div class="row" style="color: #1f49aa;">
      ...
      <!--END OF V7 SKILLS MAP-->
         {{ Form::model($skills, array('url'=>'', 'class'=>'form-horizontal', 'id'=>'', 'role'=>'form', 'method' => 'post')) }}
         <!--start wrapper v8 -->
            ...
            <table class="table" style="background: #FFF;" height="400">
               <thead>
                  <tr align="center" style="background: #9FA7B4;">
                     <th colspan="2"> <a href="#viewFeedbacksModal" style="color: #FFF;" data-toggle="modal">No. of Feedbacks ( <span id="span-no-feedback">{{ $no_of_feedbacks }}</span> )</a></th>                     
                  </tr>
               </thead>
               <tbody>
                  <tr id="tr-feedback">
...
<div id="feedbackModal" style="overflow-y: auto;"  class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
   ...
   <div class="modal-body">
      <script type="text/javascript" src="https://ajax.googleapis.com/ajax/libs/jquery/1.7.2/jquery.min.js"></script>
      {{ HTML::script('resources/js/ddslick.js') }}     
      {{ Form::open(array('url'=>'', 'class'=>'form-horizontal', 'autocomplete'=>'off', 'id'=>'', 'role'=>'form', 'method' => 'post')) }}
      <table style="color: #1f49aa; font-size: 13px;" >
         <tr>
            <td>Customer</td>
            <td>
      ...
      <table cellpadding="3" class="table-header-rotated">
         ...
         <tbody>
            <tr id="tr-feedback-{{ $cust_feed->id }}">
               ...
               <td class="fu-td"><input style="width: 40px;" readonly type="text" value="{{ $cust_feed->communication }}"></td>
               <td class="fu-td"><input style="width: 40px;" readonly type="text" value="{{ $cust_feed->commitment }}"></td>
               <td class="fu-td"><input style="width: 40px;" readonly type="text" value="{{ $cust_feed->analysis }}"></td>
               <td class="fu-td"><input style="width: 40px;" readonly type="text" value="{{ $cust_feed->delivery }}"></td>
               <td class="fu-td"><input style="width: 40px;" readonly type="text" value="{{ $cust_feed->productivity }}"></td>
               <td class="fu-td"><input style="width: 40px;" readonly type="text" value="{{ $cust_feed->fixing }}"></td>
               <td class="fu-td"><input style="width: 40px;" readonly type="text" value="{{ $cust_feed->presentability }}"></td>
               <td class="fu-td"><input style="width: 40px;" readonly type="text" value="{{ $cust_feed->recommendation }}"></td>               
            ...
            if(confirm('Are you sure you want to permanently delete this feedback?')) {
               $.ajax({
                   ...
                   success: function (response) {
                        ...
                        window.location.href= "{{ URL::to('admin/skills-map/update?show=voc&id='.$_GET['id'].'#scroll-voc') }}";
                        /*
...
<?php
   function dropDown($name, $value) {      
      ...
      $html = '<input type="text" style="width: 30px;" readonly value="'.$value.'">';
