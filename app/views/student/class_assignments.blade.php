					...
					<table class="table table-striped">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									@if ($assignment->assignment_student_id == NULL)
										{{ (date('Y-m-d') <= $assignment->due_date) ? 'Open' : 'Closed' }}
									@else
										Done
									@endif
								</td>
								<td>
									...
									@if (date('Y-m-d') <= $assignment->due_date && $assignment->assignment_student_id==NULL)
									<a href="{{ $url_class . '/assignments/submit?id=' . $assignment->id }}" class="glyphicon glyphicon-floppy-disk" title="Submit Assignment"></a>&nbsp;&nbsp;
									@endif
								</td>
							</tr>
							@endforeach
