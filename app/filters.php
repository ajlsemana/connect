...
{
	if (!Auth::check()  || ! in_array(Auth::user()->user_type, array(1, 3, 5))) {
		if (Request::ajax()) {
			return Response::json(array('error' => 'Your session has expired. Please log in again'), 403);
		} else {
			return Redirect::guest('auth/login');
...
Route::filter('auth.faculty', function()
{
	if (!Auth::check() || Auth::user()->user_type != 2) {
		if (Request::ajax()) {
			return Response::json(array('error' => 'Your session has expired. Please log in again'), 403);
		} else {
			return Redirect::guest('faculty/login');
		}
	}
});
...
{
	if (Auth::check() && Session::getId() != Auth::user()->session_id) {
